...
const OTPVerification = () => {
  ...
  const handleResendOTP = async () => {
    ...
    try {
      ...
      // Use the OTP generation endpoint
  ...
  const handleVerifyOTP = async () => {
    ...
    try {
      ...
      if (response.data.isNewUser) {
        ...
        router.push('/rider/Auth/PersonalInformation');
      } else {
        // Check if the existing user is a rider
        const userRole = response.data.user.role;
        
        if (userRole === 'rider') {
          // Existing rider, store tokens and user info
          ...
          // User exists but is not a rider
          Alert.alert(
            'Account Type Mismatch',
            'This phone number is already registered as a customer. Please use a different phone number to register as a rider.',
            [
              {
                text: 'OK',
                onPress: () => router.replace('/rider/Auth/PhoneInput')
              }
            ]
          );
        }
